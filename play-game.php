<?php
    ...
    if ($stmt->rowCount() > 0) {
        foreach ($stmt as $row) {
            ...
            $query = "UPDATE games SET plays=? WHERE id=?";
            $stmt = $pdo->prepare($query);
            $stmt->execute([$plays, $gameid]);
        }
    } else {
        echo "<a href=\"/\">Go Back</a>";
        die("<script>alert(\"No game found.\");</script>");
    }
...
if(isset($_COOKIE["WARNING-DO-NOT-SHARE-WATERGAME-USCID"])){
    ...
    $query = "INSERT INTO logs (id, userid, gameid) VALUES (NULL, ?, ?)";
    $stmt = $pdo->prepare($query);
...
<iframe src="/game-files/<?php echo $url; ?>index.html" class="game"></iframe>
...
<footer id="bottom-bar">
    ...
    <a class="nav-item" href="#"><i class="fab fa-internet-explorer fa-xl"></i></a>
    <a class="nav-item" href="#"><i class="fas fa-music fa-xl"></i></a>
